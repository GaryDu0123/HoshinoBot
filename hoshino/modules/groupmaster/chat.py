...
from hoshino.config import VERSION
...
async def say_version(bot, ev):
    await bot.send(ev, f'当前版本 {VERSION}')
...
async def chat_waifu(bot, ev):
    if not priv.check_priv(ev, priv.SUPERUSER):
        if random.random() < 0.8:
            await bot.send(ev, R.img(f'laopo{random.randint(1, 3)}.jpg').cqcode)
            await bot.send(ev, '不要乱叫别人老婆啊（', at_sender=True)
        else:
            await bot.send(ev, R.img(f'laopo{random.randint(4, 5)}.jpg').cqcode)
    else:
...
async def chat_laogong(bot, ev):
    await bot.send(ev, '你给我爬！', at_sender=True)
